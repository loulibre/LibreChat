...
# Known text file extensions (overrides mime type detection)
TEXT_EXTENSIONS = {
    ...
    '.vim', '.conf',  # Configuration files
    '.env', '.ini'  # Environment and config files
}
...
def is_text_file(file_path):
    try:
        ...
        if os.path.getsize(file_path) > FILE_SIZE_LIMIT:
            return False
        # MIME type check (skip for known text extensions)
        mime_type = mimetypes.guess_type(file_path)[0]
        if mime_type and not mime_type.startswith('text/') and ext not in TEXT_EXTENSIONS:
            return False
        # UTF-8 check
        ...
        print(f"Error checking if file is text: {file_path} - {str(e)}")
        return False
